...
class MyFrameQ3Skel extends JFrame implements ActionListener {
    ...
    public static int addEqual(ArrayList list, int tar) {              // to be completed
        if (list.size() == 0) return 0;
        else {
            int first = (Integer) list.get(0);
            list.remove(0);
            if (first == tar) return first + addEqual(list, tar);
            else return 0 + addEqual(list, tar);
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (target == b2) {
            ...
            int res = addEqual(new ArrayList(list), tar);
            t3.setText("" + res);
        }
        if (target == b4) {
            int tar = Integer.parseInt(t22.getText());
